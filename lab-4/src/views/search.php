...
if ($search && $category_id && $category_id != 'all') {
    $products = $product_repository->getBySearchWithCategoryId($search, $category_id, $offset, $limit);
} else if ($category_id && $category_id != 'all') {
    $products = $product_repository->getByCategoryId($category_id, $offset, $limit);
} else if ($search) {
    $products = $product_repository->getBySearch($search, $offset, $limit);
} else {
    $products = $product_repository->getAll($offset, $limit);
}

$category_repository = new CategoryRepository($pdo);
